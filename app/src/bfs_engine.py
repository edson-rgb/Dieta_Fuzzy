...
# Vizinhança do IMC (categorias ordenadas)
vizinhos_imc = {
    "baixo": ["normal"],
...
# Vizinhança da intensidade (categorias ordenadas)
vizinhos_intensidade = {
    "baixa": ["moderada"],
    "moderada": ["baixa", "alta"],
    "alta": ["moderada"],

    # Categorias extras (fallback inteligente)
    "muito_baixa": ["baixa"],
    "muito_alta": ["alta"]
}
...
def gerar_filhos(estado):
    """
    Gera estados filhos alterando IMC OU Intensidade por vez.
    """
...
def bfs_busca_dieta(imc_inicial, intensidade_inicial):
    ...
    # Se nada for encontrado (teoricamente impossível)
    return {
